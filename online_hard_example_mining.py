...
def online_hard_example_mining(logits, targets, default_boxes, match, nms_threshold, config):
    ...
    # 这个值不需要计算梯度
    train_match = torch.zeros((batch_size, num_default_boxes), requires_grad=False).long()
    ...
    # 这里将损失函数值还原成原来的形状
    loss = loss.view(batch_size, num_default_boxes, 4)
    # 这里原来论文中并没有说清楚，这个OHEM的loss score是怎么来的，因为一个default box的损失函数值有4个，这里我们默认是求和
    # shape [batch_size, num_default_boxes]
...
def nms(boxes_with_scores, nms_threshold):
    ...
    for i in range(num_boxes - 1):
        ...
        iou = area_overlap / (area[order[i].data] + (y2[inds] - y1[inds] + 1) * (x2[inds] - x1[inds] + 1) - area_overlap)
    ...
    for i in range(batch_size):
        ...
        loss_scores = loss[i, :][ixs.data]
        loss_scores, order = torch.sort(loss_scores, descending=True)
        
        fc = 0
        bc = 0
        j = 0
        while (fc < actual_fg_count or bc < actual_bg_count) and j < num_positive_negative_boxes:
            index = ixs[order[j].data].data
            if keep[i, index] == 1 and fc < actual_fg_count:
                train_match = 1
                fc += 1
                
            elif keep[i, index] == -1 and bc < actual_bg_count:
                train_match = -1
                bg += 1
            else:
